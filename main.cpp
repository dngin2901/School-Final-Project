...
unsigned int movesBeforeGrowth = INT16_MAX;
...
// Check if partition
// Check the traveler segment array against pos which will handle collision detection with itself
// ,other travelers and walls.
bool canMove(GridPosition pos, Traveler* trav) {

	if (pos.row > numRows - 1 || pos.row < 0 || pos.col > numCols - 1 || pos.col < 0) { return false;}
	for (int i = 0; i < trav->segmentList.size(); i++) {
		travelerLock.lock();
		if (trav->segmentList[i].col == pos.col && trav->segmentList[i].row == pos.row) {
			travelerLock.unlock();
			return false;
		}
		travelerLock.unlock();
	...
	travelerLock.lock();
	if (grid[pos.row][pos.col] != SquareType::FREE_SQUARE && grid[pos.row][pos.col] != SquareType::EXIT) {
		travelerLock.unlock();
		return false;
	}
	else {
...
// This function looks all around the traveler and selects a 
//random move and returns that move to update traveler
Direction getNextPosition(Traveler* traveler) {

	travelerLock.lock();
	unsigned int headCol = traveler->segmentList[0].col;
	unsigned int headRow = traveler->segmentList[0].row;
	travelerLock.unlock();
	std::vector<GridPosition> possibleMoves;
	std::vector<GridPosition> actualMoves;
	...
	North.col = headCol; North.row = headRow - 1;
	South.col = headCol; South.row = headRow + 1;
	East.col = headCol + 1; East.row = headRow;
	West.col = headCol - 1; West.row = headRow;

	possibleMoves.push_back(North); possibleMoves.push_back(South); possibleMoves.push_back(East); possibleMoves.push_back(West);

	for (int i = 0; i < possibleMoves.size(); i++) {
		
		if (canMove(possibleMoves[i], traveler)) {
			actualMoves.push_back(possibleMoves[i]);
		}
	}

	if (actualMoves.empty()) {
		return Direction::NULL_DIR;
	}
	
	int moveChoice = rand() % actualMoves.size();

	if (actualMoves[moveChoice].col == North.col && actualMoves[moveChoice].row == North.row) {
		return Direction::NORTH;
	} else if (actualMoves[moveChoice].col == South.col && actualMoves[moveChoice].row == South.row) {
		return Direction::SOUTH;
	} else if (actualMoves[moveChoice].col == East.col && actualMoves[moveChoice].row == East.row) {
...
// This function is for updating the travelers position
void updatePosition(Traveler* traveler) {

	Direction headDir = getNextPosition(traveler);
	TravelerSegment newHead;
	switch(headDir) {
		case Direction::NORTH:
			travelerLock.lock();
			newHead.col = traveler->segmentList[0].col; newHead.row = traveler->segmentList[0].row - 1;
			travelerLock.unlock();
			newHead.dir = Direction::NORTH;
			break;
		case Direction::SOUTH:
			travelerLock.lock();
			newHead.col = traveler->segmentList[0].col; newHead.row = traveler->segmentList[0].row + 1;
			travelerLock.unlock();
			newHead.dir = Direction::SOUTH;
			break;
		case Direction::EAST:
			travelerLock.lock();
			newHead.col = traveler->segmentList[0].col + 1; newHead.row = traveler->segmentList[0].row;
			travelerLock.unlock();
			newHead.dir = Direction::EAST;
			break;
		case Direction::WEST:
			travelerLock.lock();
			newHead.col = traveler->segmentList[0].col - 1; newHead.row = traveler->segmentList[0].row;
			travelerLock.unlock();
			newHead.dir = Direction::WEST;
			break;
	...
	travelerLock.lock();
	traveler->segmentList.push_front(newHead); traveler->moveCount++;
	travelerLock.unlock();

	// Makes the tail grow every N moves.
	if (traveler->moveCount % movesBeforeGrowth != 0) {
		travelerLock.lock();
...
void updateTraveler(int n) {

	using namespace std::chrono_literals;
	while(true) {
		updatePosition(&travelerList[n]);
...
void threadWork() {
	...
	for (int i = 0; i < numTravelers; i++) {
		travelerThreads[i] = std::thread(updateTraveler, i);
	}
}
...
void drawTravelers(void)
{
	...
	//-----------------------------
	std::cout << "drawTravelers" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	if (argc > 4) {movesBeforeGrowth = std::atoi(argv[4]);}
	numLiveThreads = 0;
	numTravelersDone = 0;
	...
	for (int i = 0; i < numTravelers; i++) {
		std::thread temp;
		travelerThreads.push_back(move(temp));
	}
...
cout << endl;
	...
	// where the threads will be created 
	threadWork();
